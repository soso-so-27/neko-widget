    ...
    def test_actual_workflow_keeps_app_ui_in_first_five_without_losing_evidence(self):
        ...
        for selected in scope.SCOPES:
            with self.subTest(scope=selected):
                ...
                self.assertEqual(maximum_running, 4 if selected in (scope.PHOTO_SCOPE, scope.OFFICIAL_SCOPE, scope.COMBINED_SCOPE, scope.REVIEWED_APP_SCOPE, scope.REVIEWED_WIDGET_ENTRY_SCOPE) else 5)
                if selected in (scope.PHOTO_SCOPE, scope.OFFICIAL_SCOPE, scope.COMBINED_SCOPE, scope.REVIEWED_APP_SCOPE, scope.REVIEWED_WIDGET_ENTRY_SCOPE):
