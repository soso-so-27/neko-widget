...
LOCAL_PHOTO_LIBRARY_USAGE_DESCRIPTION='猫の写真を端末内で見つけて写真アプリの「うちの子」アルバムとウィジェットへ反映し、猫が写る写真や動画から季節のムービーを再生するため、写真ライブラリへのアクセスを許可してください。解析と再生は端末内で行い、元の写真や動画を変更・削除しません。'
