...
このpackは、共有機能を含む現行sourceから`disabled` releaseを作る場合にも、共有runtimeが完全に
無効であることをarchiveで確認して使う。Build 36の署名dry runは過去の記録であり、現在のHEADや
最終archive evidenceを証明しない。公開Privacy／Supportページの問い合わせ経路、所有者入力、
現在の提出候補SHAによる最終`disabled` uploadの証拠が揃うまで、提出可否は`RED`のままとする。
...
```text
猫写真を端末内で見つけ、毎日のウィジェットへ。「思い出」に残した1枚を書き出したり、1〜30枚をPDFにまとめたりできます。
```
...
・ホーム画面のウィジェットに一枚を表示
・「写真」の「自動アルバム」で年ごと、成長、どアップ、人といっしょ、おでかけなどを振り返る（写真の内容や撮影情報に応じて表示）
・気に入った写真を「思い出に残す」で選び、「思い出」にまとめる
・「思い出」の一枚を、位置情報などの元情報を除いた最大2,048pxのJPEGに書き出す
・「思い出」から1〜30枚を選び、PDFに書き出す
...
一枚のJPEGやPDFなどを書き出すときだけiOSの共有シートが開き、共有先は利用者が選びます。単写真JPEGはアプリ自身が管理する書き出し用の一時ファイルを作らず画像データとして渡しますが、iOSまたは共有先がコピーを作成・保持する場合があり、その共有先のポリシーが適用されます。
```
...
3. 「写真」の一覧で一枚を開き、「思い出に残す」を押すと「思い出」に残ります。
4. 「写真」の「自動アルバム」から、利用可能な写真に応じた分類を開けます。
5. 「思い出」の一枚のメニューで「写真を書き出す」を選ぶと、位置情報などの元情報を除いた最大2,048pxのJPEGを端末内で作成し、iOSの共有シートを開けます。
6. 「思い出」で1〜30枚を選ぶと、端末内でPDFを作成し、iOSの共有シートを開けます。
7. ホーム画面の空白を長押しし、「編集」から「ウィジェットを追加」を選び、「ねこのまど」を追加します。WidgetKitの更新時刻はiOSが決めるため、操作直後に画像が切り替わらない場合があります。

プライバシーと機能境界
・猫判定、一覧、ウィジェット用画像、単写真JPEG、PDF作成は端末内で行います。
・許可した写真だけを読み、元写真を削除・移動しません。
・このビルドでは、ほかの利用者とのネットワーク写真共有、招待、送信、受信、届いた写真の一覧を無効にしています。
・開発者サーバーへの自動通信、サインイン、アカウント、広告、トラッキングはありません。
・利用者が単写真JPEG、PDF等の書き出しを選んだ場合だけiOSの共有シートを開きます。単写真JPEGは位置情報、撮影日時、元のファイル名などの元情報を除き、アプリ自身が管理する書き出し用の一時ファイルは作りません。iOSまたは共有先がコピーを作成・保持する場合があり、共有先のポリシーが適用されます。
