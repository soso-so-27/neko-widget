# ADR-004：Widget肉球ボタンとBuild 6計測

- 状態：承認済み
- 日付：2026-08-16
- 対象：TestFlight Build 6以降
...
Build 6をiPhoneへインストールして下記の実機ゲートを終えた後、アプリ内の「1週間計測を開始」を押した時点から行動計測を開始する。ゲート中の試し押しは計測へ含めない。
...
1週間計測を始める前に、TestFlight上で次をすべて確認する。
...
- Build 5で配置済みのWidgetを残したままBuild 6へ更新し、表示と長押し編集が動く。不調なら削除・再追加して結果を記録する。
