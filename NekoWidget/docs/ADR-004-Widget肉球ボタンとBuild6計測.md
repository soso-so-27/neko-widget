...
> **Superseded（利用者向け語彙、2026-08-26）：** この文書の「肉球」「これ好き」「Like」はBuild 6当時の履歴である。現行画面では自分の写真を選ぶ操作を「思い出に残す」と表示し、相手へ返す「ハートを送る」と分離する。現行仕様は[情報設計](情報設計.md)、決定経緯は[ADR-020](ADR-020-思い出とハートの操作分離.md)と[ADR-021](ADR-021-受信写真の恒久的な思い出取込.md)を参照する。内部型名と移行用schema名は互換性のため残り得る。
...
Build 6で肉球操作と保存境界を導入し、Build 7で[ADR-005](ADR-005-Widget猫優先full-bleed.md)の表示修正を入れた。Build 7ではWidget再配置とランダム100枚のPrecision確認を完了し、`reviewNo 74`だけを製品候補から除外して99 / 100を採用した。scannerはBuild 8で変更しないため、この標本は再利用する。
...
Simulator CIはSwiftのコンパイルと通常のApp Group入出力を検証するが、ホーム画面へ配置したWidgetの肉球タップと、別processで保存した直後のアプリ表示までは自動化しない。このゲートは実機で行う。
