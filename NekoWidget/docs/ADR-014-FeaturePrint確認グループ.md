...
> Build 18の実機896枚では、利用者が何十回も「混ざってる」で分割しても個体分類を完了できなかった。FeaturePrintは猫の個体差より、背景、姿勢、明るさ、構図、猫の大きさ、子猫か成猫かという意味的な見た目へ強く影響された。これは閾値調整で解消する問題ではないため、Build 19で確認グループのUI導線を非表示にした。
>
> 保存済みの明示的なプロフィール所属は削除しない。FeaturePrintのservice、検証、privacy境界も将来の再評価に備えて残すが、現行製品から利用者が起動できる導線は持たない。
>
> 次の再評価は同じ教師なしgroupingの再開ではなく、[ADR-016](ADR-016-猫個体5-shot分離実験.md)の2匹×5枚・unknown付き診断実験として行う。実機数値がGO条件を満たすまで製品UIを作らない。
