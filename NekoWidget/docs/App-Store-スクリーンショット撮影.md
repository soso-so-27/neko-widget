...
| 順 | ファイル | 実際に示す機能 |
| --- | --- | --- |
| 1 | `01-on-device-photo-privacy.jpg` | 写真は端末外へ出さず、端末内で解析する許可前の説明 |
| 2 | `02-private-photo-window.jpg` | 端末内で選ばれた「思い出の一枚」と肉球 |
...
- 画面本体は`OnboardingView`、`MainTabView`、`HomeView`、`AlbumView`、`LikedPhotosView`という
  製品UIをそのまま使う。存在しない機能やマーケティング用の上書きUIは足さない。
...
Appleの6.9-inch iPhone portrait accepted sizeのいずれか1種類で4枚を統一する。
...
日本語、ライト表示、09:41、満充電へ固定して1つのUI testだけを実行する。成果物の保持期間は14日。
