...
ソースはWindows上で作成しています。GitHub ActionsではXcodeコンパイル、Simulator上の起動・PhotoKit・Vision・App Groupスモークテスト、1,000枚のスケール／メモリテストまで成功しています。iPhoneではBuild 7までの技術検証に加え、Build 8のMedium / Large表示不具合まで確認し、8,861枚の確定スキャンとdetected無作為100枚のレビューを完了しました。レビューは`reviewNo 74`だけを製品候補から除外し、99 / 100を採用しました。scannerはBuild 10でも変更しないため、このPrecision標本は再利用します。Build 7の1週間計測はLike表示不具合で中断し、Build 8では高解像度20件TimelineによりMedium / Largeがplaceholder相当になる不具合を確認しました。Build 9でTimelineを最大2件へ制限し、Build 10で写真ブラウザの標準ページングを修復しました。3サイズ、like／unlike即時反映、写真ブラウザの操作感を実機確認した後だけ、新しいbaselineから計測を始めます。計測中はアルバム／共有とdeferred probeを別branchで開発・CIまで進めても、測定端末へインストールしません。実施順は[実機技術検証チェックリスト](docs/実機技術検証チェックリスト.md)、Like修復は[ADR-007](docs/ADR-007-Build8計測修復と最終UX.md)、Timeline修復は[ADR-008](docs/ADR-008-高解像度WidgetのTimeline負荷制限.md)、アルバム／共有は[ADR-009](docs/ADR-009-ローカルアルバムと招待制共有.md)、898件の遅延pagingは[ADR-010](docs/ADR-010-大規模写真ブラウザの遅延ページング.md)で管理します。
...
実機8,861枚のうちScreenshot 736枚と`unavailableLocally` 2,586枚を除く解析済みは5,539枚で、猫898枚は解析済み集合の16.2%です。Deferredにも同率で猫がいる単純仮定では未発見約419枚、潜在約1,317枚、観測率約68.2%になります。現行の`unavailableLocally`は1024px high-quality requestをローカルだけで満たせなかった意味であり、低解像度派生も存在しないとはまだ断定しません。Build 10にもdownload処理やscanner request変更は入れず、現状の898枚で1週間計測を取り直します。512px fast-formatの非破壊probeは1週間後のInternal技術検証buildで実行し、採用時だけ後続production buildへ反映します。番号は計測終了時に未使用番号を割り当てる方針を[ADR-006](docs/ADR-006-iCloudローカル派生画像の検証.md)へ記録しています。
