...
端末の写真ライブラリをオンデバイスで調べ、猫が主役の写真を選別するiOS 17.1以上向けSwiftUIアプリです。選別結果は「うちの子」アルバムと、Small / Medium / LargeのWidgetKitウィジェットへ渡します。自分で選ぶ「思い出」の記録はApp Group内へ永続化します。アプリの「写真」はこのiPhoneで見つけた猫写真を見る場所、利用者が名前を付ける「まど」は招待した相手1人との非公開な共有空間です。本人所有2台の内部TestFlightでは、名前付きの非公開なまど1つ・2人に限定したE2E暗号化の一枚共有も検証しています。
...
「届いた写真」の「思い出に追加」は、利用者が明示的に選んだ一枚を、位置情報を除いて写真アプリへ取り込み、通常の「思い出」、PDF、将来の本へ加える操作です。相手には通知せず、「思い出から外す」、共有解除、ブロック、アプリ削除後も写真アプリの写真自体は削除しません。相手へ送る「ハート」とは別の操作です。詳細は[ADR-021](docs/ADR-021-受信写真の恒久的な思い出取込.md)にあります。
...
- アプリが前面、背面、終了中の各状態でウィジェットをタップし、`nekowidget://photo?id=...&shownAt=...`から該当写真の詳細が開くことを確認します。大きな写真、左右スワイプ、思い出のブックマーク、撮影日、「この日の写真をすべて見る」、約20分ごとの切り替えと最後に変わった時刻が表示され、「次へ」ボタンがないことも確認します。
- 3サイズの右下に思い出のブックマークがあり、未保存からはアプリを開かずに追加でき、保存後は安定した塗りつぶし表示になることを確認します。誤解除を避けるため、解除はアプリ内のメニューと確認画面から行います。ブックマーク以外の領域では従来のDeep Linkが働きます。まどの写真ではハートが別に表示され、送信待ちとサーバー受付済みを区別します。
