...
CREATE TABLE pa_retention (
  ...
  CHECK ((expired_at IS NULL AND due_at IS NULL AND paused_at IS NULL AND final_notice_delivered_at IS NULL)
    OR (expired_at IS NOT NULL AND due_at IS NOT NULL))
);
