...
`src/s3-version-purge.ts` は通常のwriterと別の資格情報・明示flagを要求する、版IDを指定したS3削除要求の孤立部品。公開Workerやcronからの呼出しはなく、現時点で写真は消さない。204応答も物理消去完了とはせず、独立した作業台帳・新鮮な課金/通知証拠・全版再一覧で不在を確認する実行経路が別途必要。実IAM削除権限、Object Lock、実S3での削除試験は未確認。
...
## Owner復旧一覧の可逆圧縮（書込み既定OFF）

`OWNER_RECOVERY_COMPRESSION_ENABLED=YES` の場合のみ、新しいowner復旧一覧をgzip圧縮してから既存のowner/purpose認証付き暗号化へ渡す。写真・メモ本体・データ鍵・世代番号・履歴の保持は変更しない。4KiB未満、または10%以上縮まらない一覧は従来JSONのまま保存する。形式は暗号化の内側で `NKZ1`＋展開長（uint32BE）＋gzip。展開は元の21MiB上限、宣言長、処理時間、chunk数を制限し、不明形式・破損・長さ不一致は503で拒否する。

導入順序は**全reader更新 → 合成データで新旧形式の復元確認 → 書込みフラグをYES**。この変更だけで遠隔フラグやサービス公開を有効にしない。新readerはフラグNOでも両形式を読めるため、圧縮書込み停止はNOへ戻せばよい。圧縮済みデータが一つでもある間は、このreader導入前のコードへrollbackしない。既存objectの再圧縮・上書き・削除は行わない。

これは一覧1版のbyte数を減らす変更で、更新ごとに全一覧を保存する累積増加や写真・旧版の請求を解消しない。実運用費・提供容量の確約には全版の実測が必要。
