...
import { boundKeyCustody, boundMembership, boundPhotoValidator } from './providers';
...
export interface Env {
  ...
  KEY_CUSTODY?: Fetcher; MEMBERSHIP_AUTHORITY?: Fetcher; PHOTO_VALIDATOR?: Fetcher;
...
async function body(request: Request, maximum: number): Promise<Record<string, unknown>> {
  ...
  try {
    while (true) {
      ...
      size += value.length;
      if (size > maximum) { await reader.cancel(); throw new ServiceError('REQUEST_TOO_LARGE', 413); }
      chunks.push(value);
    }
...
function configuredServices(env: Env): Services {
  if (!env.KEY_CUSTODY || !env.MEMBERSHIP_AUTHORITY || !env.PHOTO_VALIDATOR || !env.IDENTITY_INDEX_SECRET
  ...
  const now = () => Date.now();
  const keys = boundKeyCustody(env.KEY_CUSTODY);
