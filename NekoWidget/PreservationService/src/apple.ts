...
async function boundedBody(response: Response): Promise<string> {
  try {
    return new TextDecoder('utf-8', { fatal: true }).decode(await readBoundedBody(response.body, MAX_RESPONSE_BYTES,
      () => new ServiceError('APPLE_RESPONSE_INVALID', 502)));
  } catch { return fail('APPLE_RESPONSE_INVALID'); }
}
