...
App Store Server Notification Historyは、公開webhookとは分離したprivate verifier経由の欠落回復だけに使います。復旧期間を固定し、5分ごとに最大1ページ（20件）だけ取得し、各通知の正規化済み台帳と一度きりの再照合原因が永続化された後にだけcursorを進めます。途中停止時は同じページを冪等に再実行し、生のApple JWSはWorker・D1へ渡しません。専用の上限・下限runtime gateは既定OFFで、cursor失効、設定不一致、検証済みresponseの矛盾は自動で期間を変更せずblockedにします。
