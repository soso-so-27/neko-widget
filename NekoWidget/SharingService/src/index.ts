...
import {
  ...
  getWindowSponsorshipGrant,
} from "./billing-window-sponsorship";
import {
...
export async function route(
  ...
  if (request.method === "DELETE" && pathname === "/v1/window-sponsorship") {
    return detachWindowSponsorshipAsOwner(request, env);
  }
  ...
  const approveMatch = pathname.match(
    /^\/v1\/pairing\/enrollments\/([^/]+)\/approve$/u,
  );
  ...
  const completeMatch = pathname.match(
    /^\/v1\/pairing\/enrollments\/([^/]+)\/complete$/u,
  );
  ...
  const cancelMatch = pathname.match(
    /^\/v1\/pairing\/enrollments\/([^/]+)\/cancel$/u,
  );
  ...
  if (
    request.method === "GET" &&
    pathname === "/v2/device-recoveries/pending"
  ) {
  ...
  if (request.method === "GET" && recoveryDescriptorMatch?.[1] !== undefined) {
    return getDeviceRecoveryDescriptor(
      request,
      env,
      recoveryDescriptorMatch[1],
    );
  }
  ...
  if (
    request.method === "GET" &&
    recoverySponsorStatusMatch?.[1] !== undefined
  ) {
    return getSponsorDeviceRecoveryStatus(
      request,
      env,
      recoverySponsorStatusMatch[1],
    );
  }
  ...
  const pawReactionMatch = pathname.match(
    /^\/v2\/moments\/([^/]+)\/reactions$/u,
  );
  if (
  ...
  const participantBlockMatch = pathname.match(
    /^\/v2\/participants\/([^/]+)\/block$/u,
  );
  ...
  if (
    request.method === "POST" &&
    pathname === "/v1/sharing/generations/reserve"
  ) {
    return reserveGeneration(request, env);
  }
  const descriptorMatch = pathname.match(
    /^\/v1\/sharing\/generations\/([^/]+)\/descriptors$/u,
  );
  ...
  const mediaMatch = pathname.match(
    /^\/v1\/sharing\/generations\/([^/]+)\/media\/([^/]+)$/u,
  );
  if (
    request.method === "PUT" &&
    mediaMatch?.[1] !== undefined &&
    mediaMatch[2] !== undefined
  ) {
    return uploadMedia(request, env, mediaMatch[1], mediaMatch[2]);
  }
  const prepareMatch = pathname.match(
    /^\/v1\/sharing\/generations\/([^/]+)\/prepare$/u,
  );
  ...
  if (
    ...
    return uploadManifest(
      request,
      env,
      manifestUploadMatch[1],
      manifestUploadMatch[2],
    );
  }
  const commitMatch = pathname.match(
    /^\/v1\/sharing\/generations\/([^/]+)\/commit$/u,
  );
  ...
  const currentMatch = pathname.match(
    /^\/v1\/sharing\/sources\/([^/]+)\/current$/u,
  );
  if (request.method === "GET" && currentMatch?.[1] !== undefined) {
    return getCurrent(request, env, currentMatch[1]);
  }
  const manifestDownloadMatch = pathname.match(
    /^\/v1\/sharing\/generations\/([^/]+)\/manifest$/u,
  );
