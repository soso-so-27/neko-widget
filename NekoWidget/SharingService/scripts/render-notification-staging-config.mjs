...
const outputPath = join(projectDirectory, "wrangler.notification-staging-on.jsonc");
...
try {
  ...
  console.log("Created ignored notification staging config with moment/reaction/window-name/APNs ON.");
  console.log("APNs secrets must be installed separately; no deployment was performed.");
  console.log("No Cloudflare resource identifier or secret was printed.");
} catch (error) {
  console.error(
    `notification staging config render: FAIL: ${error instanceof Error ? error.message : "unknown error"}`,
  );
  process.exitCode = 1;
}
