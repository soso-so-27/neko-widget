...
const outputPath = join(projectDirectory, "wrangler.general-staging-apns-off.jsonc");
...
try {
  ...
  console.log("Created ignored general staging APNs-OFF config with media and report ingestion ON.");
  console.log("APNs and legacy sharing remain OFF; no deployment was performed.");
  console.log("No Cloudflare resource identifier or secret was printed.");
} catch (error) {
  console.error(
    `general staging APNs-OFF config render: FAIL: ${error instanceof Error ? error.message : "unknown error"}`,
  );
  process.exitCode = 1;
}
