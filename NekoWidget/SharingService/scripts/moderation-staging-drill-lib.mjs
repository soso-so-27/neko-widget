...
export function createDrillFilesInValidatedDirectory(
  ...
  try {
    ...
    bundle = bundleFactory(recipientPublicRaw);
...
function verifyFixedSyntheticMetadata(metadata) {
...
function readAndVerifyFixedSyntheticBundle(directory) {
  let metadataBytes;
  let envelopeBytes;
  try {
...
export function verifySyntheticDrillBundleDirectory(directory) {
  requireExactDirectoryFiles(directory, [
    MODERATION_STAGING_DRILL_CIPHERTEXT_FILENAME,
    MODERATION_STAGING_DRILL_METADATA_FILENAME,
  ]);
  const bundle = readAndVerifyFixedSyntheticBundle(directory);
  try {
...
export function verifySyntheticDrillReviewDirectory(directory) {
  requireExactDirectoryFiles(directory, [
...
export function verifyCompletedSyntheticDrillDirectory(directory) {
  const expected = [
    MODERATION_STAGING_DRILL_AUDIT_FILENAME,
    MODERATION_STAGING_DRILL_METADATA_FILENAME,
  ].sort();
...
export function generateStagingModerationDrillFiles({
  ...
  const validatedPublicKey = validateExistingRestrictedFile(
    publicKeyFile,
    { expectedFilename: MODERATION_PUBLIC_FILENAME, expectedBytes: 43 },
  );
  runWindowsSecurityPhase("ValidateKeyDirectory", validatedPublicKey.directory);
