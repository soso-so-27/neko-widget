...
## 9月13日20:22追記：猫別まどへの引き継ぎ

後続の[猫のまど追加](2026-09-13-cat-public-window.md)で、現在はどこかの猫4枚・おひるね2枚・キジ白のまど1枚。Worker版は `4efc762a-517e-478e-938d-94fabbce641c`、実配備bundleは `C:/dev/neko-cat-window-20260913/output/cat-window-20260913/edition-001/bundle`。19:05 JSTに全3まどのHTTPとJPEG hashを照合し、固定checkoutを47a5eb4へ進め、`output/runtime/current.json`を更新した。排他・配備前後の証拠は `output/runtime/runs/20260913-cat-window/` にある。上の初期bundleへ戻さない。

catalog期限は9月15日19:01 JST、次の期限保守判断は9月14日19:01 JST。新しい基点で保守判定は`no-change`、上のqueue3枚と予定は維持。キジ白のまどは `generated-tabby-nap` のみを受け付け、既存queueの別猫を自動で割り当てない。同じ猫の追加在庫は0枚で、9月20日16:42 JSTの写真期限前の補充が残る。

heartbeatの重複登録・変更は行っていない。猫別まどのアプリ対応はTestFlight162へ同梱・アップロード済み。初回の自動実行・予定写真の配信・実機での反映は引き続き未確認。

## 9月13日20:40追記：猫別の写真を補充

[今回の補充記録](2026-09-13-cat-window-supply.md)が最新。キジ白の新3枚を生成・目視確認・既存publisherで加工し、あくび1枚を猫別まどへ配信。残る2枚は9/16・9/20の09:00 JST。上の公式・お題用3予定はそのまま保持した。現在4枚/2枚/2枚、未掲載は合計5枚/猫別2枚。

実versionは `511af745-5d19-4496-83f5-bf057b804df7`。current.jsonのbundleとqueueを同時に切り替え済み。queueは `C:/dev/neko-cat-window-supply-20260913/output/cat-supply-20260913/queue.json`、catalog期限9/15 20:35 JST・次の期限保守判断9/14 20:35 JST。20:40に全3まどのHTTP/JPEG照合成功、配信後の保守判定は`no-change`。原本・旧queue・過去bundle・実行記録は保持する。
