...
- CI待機は `python NekoWidget/ci/watch-ci-run.py RUN_ID --expected-sha FULL_SHA --output RESULT_JSON` を1本だけ起動する。60秒から最大180秒へ間隔を延ばし、同じ状態・stepの変化・全ログをモデルへ繰り返し返さない。通常は開始・失敗/承認待ち・完了を報告し、経過だけの説明を重ねない。失敗時だけ最初の具体的エラーに必要なログを取得する。JSONのrunner分は実行時間合計であり、課金額やCodexトークン数ではない。
