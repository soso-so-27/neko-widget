...
- 製品 `128e2b2`、専用CI `52a1e8e`、最終候補 `8fc7782` をmainへ反映。候補は独立再レビューで実before/after 8値とworkflowハッシュの一致まで確認した。
- 候補のNode `35684198257` は19秒、型検査と21件成功（テスト2.057秒）。iOS plan `35684198203` は18秒、対象選択を成功させて全Mac jobを未実行。本線Node `35684244496` は23秒、plan `35684244551` は20秒で成功。
- 専用Nodeは候補と本線で各1回走った。証拠再利用を追加したのではなく、いずれも小さいLinuxジョブ。計runner 0.800分。Mac 0回、再試行0回。これらは課金額やCodexトークン数ではない。
- 初回CIからmain成功まで1分12秒。最初の製品候補commit（03:23:24 UTC）からは21分09秒で、CI選択の準備・レビューを含む。最初の製品ファイル作成（03:13:23 UTC）からは31分10秒。19秒だけを今回全体の作業時間とはしない。調査開始からの完全な所要時間は別途計測できていない。
- 本番の個人保管は引き続きOFF。旧backend全体の移植、本人/鍵/会員と実保存先の接続、実端末復元、容量・保持・削除の提供条件は残る。Node部品のmain反映はバックアップ提供開始ではない。TestFlightはBuild 202のまま。

次は専用保管backendの本人/owner/鍵の境界を、既存実装を壊さない別バッチとして進める。非公開のNode実行先の契約・デプロイや本番有効化は未実施で、選定と運用条件を確かめてから扱う。利用者による資料転送は不要。
