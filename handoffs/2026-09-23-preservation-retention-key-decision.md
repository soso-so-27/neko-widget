...
- 全件ZIPの現行候補はiPhoneの一時領域へ1本のファイルを作る。50GiB枠を販売する場合、端末に同程度の空きがない利用者が持ち出せなくなるため、実容量分布・端末容量不足の試験と、分割持ち出しまたは直接保存先へ流す経路が必要。小さなfixtureのZIP成功を50GiBの持ち出し保証にしない。
- 50GiBを使い切った会員が1か月で期限切れになり、指定の12か月をフルに使う極端な例では、[R2 Standardの$0.015/GB月](https://developers.cloudflare.com/r2/pricing/)だけで約$9.66の保持費が後から発生する（50GiB≒53.7GB、無料枠を多数会員で共有する規模）。独立バックアップ・KMS・通知・運用費は別。月額980円の継続率と容量分布を測らずに50GiBと12か月を同時に販売約束しない。
