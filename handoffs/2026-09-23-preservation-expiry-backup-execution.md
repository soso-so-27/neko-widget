...
- 同日14:36 JSTごろに合成写真1枚の通し候補を開始。別configの限定Vitestで、ローカルD1に合成Apple本人を準備し、アプリの保存経路から実R2・実KMS・版付き実S3へ暗号文を書き、S3単独のowner候補を検証した。続いて当該合成写真のR2 objectだけを消し、別の空のローカルD1と同じstaging R2へ隔離復元した。復元ownerは無効・連絡先とsessionなし、写真暗号文は保存前と一致。14:41開始の再実行も2試験・約15秒で成功。約1分の実行で一時IAM鍵・userと合成S3全5版を削除し、後続のS3 `recovery/v1/` 全版・delete marker一覧は空、R2全object一覧も0件・継続ページなしを確認した。型検査も成功。実Apple検証・課金状態・JPEG providerは合成／代替であり、実iPhone・別のR2 bucket/リージョン・削除台帳再適用・利用再開・12か月消去は未試験。保管Worker未配備、公開flagはOFF。既知のiOS CI 64–98分は未実行で、候補の最初の試験時刻を14:36と記録し、後で必要CIを含む総所要時間を評価する。
...
後続差分では保存時・S3単独復元候補の選定時に、内側の暗号化credentialを復号してissuer/subjectからowner本人キーを再計算する。Apple連絡先も復号し、owner境界の照合値がある場合は一致を確認する。不一致・鍵不在は保存成功にせず復元候補を隔離する。記録復元は先頭のcommit改訂1が欠ける場合も隔離。保管サービス全169試験と型検査が通過し、この差分の独立安全レビューで追加の具体的P1/P2は見つからなかった。**実KMS/S3や別環境復元の成功証拠ではない。**

次のローカル候補ではowner画像へ現行記録の全ID・改訂・削除状態・S3 markerを含むinventoryを追加し、記録の保存・編集・削除後にもowner世代のS3確認を要求する。marker未確定の記録、世代逆行、過去の記録消失・tombstone復活は復旧候補を隔離する。0015のtriggerはmarker確定時にowner世代を進め、policy ON中のmarker変更と記録の物理削除を拒否する。画像payloadは未公開の旧形式と区別する`version: 2`とした。AWSアカウント未作成・0015未適用・Worker未配備につき、旧`version: 1` owner画像は実環境に存在しない。保管サービス全171試験、型検査、対象の合成migration検証が成功し、限定差分の独立レビューで追加の具体的P1/P2は見つからなかった。**D1独立復元、削除済みownerの再適用、期限消去、実AWS/R2/2端末試験は未完成で、提供開始条件を満たさない。**

続く読み取り専用候補では、S3 owner prefixの全ページを一巡し、owner画像の現行記録一覧と全commit/delete-intent markerを突合する`OwnerArchiveRecovery`を追加。owner画像にないcommit、最新markerの欠落、未解決の削除意図、改訂・削除状態の不一致は隔離する。候補は常に無効化・再認証待ちで、D1/R2への書き戻しや利用再開は行わない。owner/recordの履歴読取は無制限の同時S3読出しを避けて逐次化した。型検査と対象19試験、限定差分の独立安全レビューに成功。ページ途中で元のwriterが動く可能性、削除済みowner台帳、実AWS、別環境復元は未検証であり、この候補を復元完了や有効化の証拠にはしない。

さらに空の**オフライン**D1とowner写真prefixが空のR2へ書き戻す`OwnerQuarantineRestore`候補を追加。全S3版の候補が揃う前には書かず、R2写真は既存keyを上書きしない条件付きPUTと読戻しhash照合を行う。D1ではownerを無効状態、連絡先・sessionなしで挿入し、現行記録とS3参照を復元する。途中失敗は対象環境を無効状態のまま残し、既存データを消して再試行しない。型検査と対象20試験、限定差分の独立安全レビューに成功。これは合成S3・テストD1/R2に限る。**旧改訂のD1履歴、owner削除台帳の再適用、実KMS/S3/R2、元のwriter停止、2端末、利用再開は未実装・未検証**であり、実環境復元完了ではない。

2026-09-25の後続候補では、隔離復元に旧改訂のS3参照・commit marker・delete-intentも書き戻し、改訂2までの局所試験と合成写真1枚の実R2/KMS/S3復元試験を通した。これらは**無効状態の隔離復元**であり、ownerの有効化や削除済みownerの復活防止は依然として含まない。また、無効ownerの現行D1写真参照とR2のowner prefixを全ページ照合する読み取り専用候補を追加。欠落・余分なobject・巡回cursor循環・照合中のD1世代変化を拒否する。対象3ファイル10試験と型検査に成功。S3の全版照合、新鮮な課金/通知の再照会、消去台帳の固定、二度目の棚卸しは未実装であり、この照合結果は削除許可ではない。
...
後続ブランチ `c2d8cad` のpush前検査では、現行の固定service scopeにない新規ファイルを含むため `full-v1` と判定。ローカル開発フローチェックは93秒で成功したが、候補iOSの実測範囲64.43〜97.92分は標準目標30分を超え、preflightは `cumulative_cost_requires_replanning` で `ready=false`。候補をまだpushせず、独立コピーと冪等削除台帳の残るbackend変更をまとめてから一度だけ必要CIを実施する。30分に収まるとは報告しない。旧成功のUI結果を未検証差分へ流用しない。
...
後続ブランチの未反映候補として、暗号化された写真と記録の各改訂・削除済み状態を版付きS3へ書き、S3の版ID・SHA-256・byte数をD1の同じ改訂に記録する経路を追加した。新規保存ではR2読戻し後にS3を確認し、S3失敗時にはD1改訂を進めない。編集・個別削除も同じ経路とし、同時更新の敗者に復旧版を結び付けない。保存APIの本番構成でS3設定が欠けると書込は503で拒否し、既存記録の閲覧・持ち出しは維持する。ローカル型検査、保管サービス151試験、既存データを入れた0013移行試験を通過。独立レビューで見つかった設定欠落時の閲覧停止、大きなメモのbase64変換失敗、既存APIが許容する非v4 record IDとの不一致は修正・再確認した。これは**記録単位の復旧コピー契約**に限る。owner/認証情報/課金リンク/期限・通知を含む整合manifest、孤児S3版の掃除、削除済み記録の過去版消去、別環境への復元は未実装・未検証であり、保管サービスの有効化条件を満たさない。実AWSの成功証拠でもない。
...
5. 削除開始と更新・課金復帰の競合を閉じる所有者単位のfenceを作る。fence後は新規保管／編集と保管復旧を止め、期限切れ判定と通知証拠を再確認する。失敗時には作業台帳を残し再開可能にする。
   通常の写真後片付けキュー `pa_pending_deletes` がそのownerに残る間はfenceを作らない。事前照会だけでなく、owner無効化と同じD1 batch内で再確認し、棚卸し中に後片付けが一次写真を変えないことを保証する。
