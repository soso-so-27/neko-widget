...
## Build 5少数枚診断の追補（2026-09-06）

- 入力確認は`pet-identity-input-diagnostic-v1`。猫Aの最初の見本1枚を読み取り、単独猫検出・切り抜き・モデル出力形式だけを確認する。別猫との比較や識別精度の評価ではない。画像と小さな切り抜きはメモリ表示だけで、共有JSONには含めない。
- 個体判定の診断は`pet-identity-onnx-diagnostic-v2`。見本は各5枚のまま、判定写真は各0〜15枚・合計1〜30枚。`evaluationCountsByCat`へ実際のA/B選択枚数を記録し、未選択の枠をnilで埋めない。選んだが読めない判定写真は原因付き保留として実際の分母に含む。
- diagnosticではprecisionPassed/coveragePassed/explorationCandidate/productValidatedをすべてfalseとし、条件達成・独立検証合格と混同させない。閾値・距離・半径計算・モデル・前処理は変更しない。heldout v1の枚数、JSONキー、判定条件は維持する。
- 同一assetの重複は拒否する。診断内の同burst・dHash類似は集計警告にとどめ、原因の状態を強制的な選び直しで消さない。厳密なheldoutの重複除外は維持する。診断の標本を独立性の証拠にはしない。
- 最初の1枚を既存の見本Aの先頭として保存する。保存済みの他の見本・他の欄は保持し、既存の40枚も同じschemaのまま復元する。写真・特徴量・診断結果の永続保存、通信、ライブラリ全走査は追加しない。取消後の遅延完了からの結果復活を拒否する。
- [UIと受け入れ条件](LOW-BURDEN-DIAGNOSIS.md)。以下のBuild 3/4記録はその時点の仕様として残し、結果に合わせて過去の基準を書き換えない。
