                ...
                if let session = store.session {
                    ...
                    if let json = session.report.json {
                        if session.decisions.isEmpty {
                            Text("まだ確認結果はありません。各一覧の下の「この○枚は猫Aだけ／猫Bだけ」で確定できます。")
                                .font(.subheadline)
                    ...
                    Text("保存済みの確認があれば、写真を分類し直さずに共有できます。")
    ...
    private var comparisonSummary: some View {
        ...
        return DisclosureGroup("改善の確認 · \(summary.reviewedWithKnownChoice)/\(summary.selected)枚") {
            VStack(alignment: .leading, spacing: 8) {
                ...
                let tiles = session.report.tileComparison
                if tiles.attemptedPhotos > 0 {
                    Text("分けて検出：\(tiles.attemptedPhotos)枚を比較・\(tiles.withheldPhotos)枚を個別確認へ")
                    Text("移した写真の確認：候補と同じA/B選択 \(tiles.withheldMatchingAOrBChoices)枚・違うA/B選択 \(tiles.withheldDifferentCatProposals)枚・AとB \(tiles.withheldBothPhotos)枚")
                    Text("離れた範囲があっても、別々の猫とは限りません。保存済みの確認は変更しません。")
                        .font(.footnote).foregroundStyle(.secondary)
                    if (tiles.statuses[CandidateTileStatus.incomplete.rawValue] ?? 0) > 0 {
                        Text("追加比較できなかった写真は元の候補のままです。1匹と確認できたという意味ではありません。")
                            .font(.footnote).foregroundStyle(.secondary)
                    }
                }
