...
struct CandidateReviewPhoto: Identifiable {
    ...
    var tileCheck: CandidateTileCheck? = nil

    // Region-level suggestions must never enter a photo-level bulk confirmation.
    var batchSuggestion: CandidateReviewChoice? {
        guard tileCheck?.withholdsCandidate != true else { return nil }
        return batchSuggestionBeforeTileCheck
    }

    var batchSuggestionBeforeTileCheck: CandidateReviewChoice? {
    ...
    var issueTitle: String? {
        if tileCheck?.withholdsCandidate == true { return "離れた検出範囲があるため、写真全体で確認" }
...
struct CandidateReviewReport: Encodable {
    let protocolIdentifier = "pet-candidate-confirmation-usability-v7"
    ...
    let method = "second-nearest-of-five-per-cat;registration-radius-filter1.25;raw-single-distance-passed-four-tile-withholding;no-ratio-gate-or-identity-acceptance-or-online-learning"
    ...
    let tileComparison: CandidateTileComparison
    ...
    init(session: CandidateReviewSession) {
        ...
        tileComparison = .init(session: session)
