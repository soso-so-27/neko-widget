...
Fixed before execution: Nano416, score >= .3, class-agnostic NMS .45,
...
def decode(output):
    if output.shape != (1, 3549, 85) or not np.isfinite(output).all():
        raise ValueError("Unexpected Nano416 output")
    grids, strides = [], []
...
def detect(session, image):
    ...
    prediction = decode(session.run(None, {session.get_inputs()[0].name: tensor})[0])
...
def main():
    ...
    result = {"scope": "generated-only-desktop-mechanism-exploration;not-pixel-identical-to-UIKit;not-device-or-product-validation",
              "model": "YOLOX-Nano-0.1.1rc0", "modelSHA256": args.sha256.lower(),
