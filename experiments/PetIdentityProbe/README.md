...
2026-09-08 / 次候補: [保存済み1枚の縮小・余白比較](DETECTOR-SCALE-COMPARISON.md)。Build 8の本人実機結果は基準画像1/1/2候補でCIと一致、保存済み1枚はraw0件。既存の1ボタンへ、条件成立時だけ形式統一した100/75/50%の比較を追加。元結果の上書きや個体識別fallbackはしない。本人の写真はリポジトリ・CIへ含めない。候補`e741ebe`の専用CIは無署名device build・57テストに成功。研究用ブランチまで反映し、次候補はまだTestFlight未配布。配布済み版は以下のBuild 8。証拠と制約は比較文書を参照。
