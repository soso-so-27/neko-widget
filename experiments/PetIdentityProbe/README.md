...
2026-09-10 / 最新配布は **Build21**。本人の21枚全確認を受領し、候補一致12/13、有用な正しい候補12/16、A+Bの単独誤候補1/5と判明した。A/B取り違えではなく、2匹写り1枚の単独候補混入を優先する。[本人結果の分母と限界](diagnostics/build21-confirmed-baseline-20260910.md)。採用目標は未達。

追加50%検出に続き、4領域の詳細検出も改善を確認できず、**Build22は未配布**。比較不能9件は画像端を越える検出枠と厳格な境界検査の不整合と切り分け、分割案は不採用にした。候補ブランチ上のv7・比較UIは配布済み機能ではない。[不採用の根拠](diagnostics/tile-check-result-20260910.md)。別方式YOLOX-NanoのPC生成画像比較では2匹画像6組中4組でCat枠2、単猫15画像の誤複数枠0。ただし実写真/iPhoneの効果は未検証で、次の限定比較候補とした。[条件・結果・実装境界](diagnostics/yolox-first-result-20260910.md)。既存21枚・本人判断・本アプリは変更しない。以下は各版当時の経緯。

2026-09-10 / Build 21準備：[採用目標と改善ループ](CANDIDATE-QUALITY-GOAL.md)を固定。2匹写りを単独候補から訂正しやすい入口、全写真で下固定の確認ボタン、新旧本人判断と現在候補の集計（共有v6）を追加する。以前のA/B判断を「1匹だけ確認済み」に読み替えない。モデル・検出・閾値・保存方式は不変で、自動検出の精度改善やゴール達成ではない。Build20の本人結果は21枚中13候補・距離保留3・複数検出4・未検出1、本人確認0。実機の質/運用ゲートは未達。

2026-09-10 / Build 20準備：候補の最上位猫の見本範囲から離れた写真は、A/Bを強制せず既存の個別確認へ回す。既存半径×1.25だけを再利用し、比率0.70は導入しない。単一写真と範囲候補へ同じ制御を適用。モデル・元の評価・保存済み本人確認は変更せず、共有v5で候補数と保留の影響を分離する。[実装範囲と未検証事項](CANDIDATE-REVIEW.md#build-20見本から離れた写真へ候補を強制しない)。実写真の精度向上はまだ未確認。

2026-09-10 / Build 19準備：独立TestFlightの先行テストを猫A/Bの2匹・1回24枚までと明示。本人確認・除外・直前取消を端末内だけに保存し、途中再開・個別取消を追加。モデルや識別精度、本アプリの複数猫機能は変更しない。Build18以前の選択は再利用するが、当時保存しなかった本人確認は復元しない。[最新の範囲と保存仕様](CANDIDATE-REVIEW.md#build-19対象を限定し確認を保存して再開)。以下は各版当時の記録であり、最新の配布完了は別途CI・Apple側で確認する。

2026-09-08 / Build 14本人結果受領済み：[候補検索方式の比較と本体採用の判断境界](REFERENCE-RANKING.md)。入力12/12、順位は従来の集約・最近傍ともA/B各1枚、計2/2で期待猫が最上位。従来の確定判定は差不足で2枚保留。Build 13のcandidate全体・slots・modelと一致し、今回の範囲に最近傍の優位性はない。方式比較はいったん終了。本体は既存の複数猫プロフィール/明示所属/一括操作/通常Photosアルバム連携を維持し、自動確定は後日リリースへ延期する。確認付き候補も実用性未検証のため公開しない。2/2を精度100%とは呼ばない。

コード `27250d5` の専用CI `34200868316` は85テスト成功・独立レビュー/画面確認完了。同じSHAの配布 `34201596512` が成功。ASCで0.1 (14)、UUID `4c8d6a5a-5302-4be4-90c4-4b7d7ed9c8f7`、作成16:57、処理「終了」、内部「テスト中」、既存グループ「自分用（猫識別検証）」への反映を確認済み。本人の比較・共有も完了。同じ2枚を再測定するBuild 15や追加操作は現在不要。今回は結果と判断文書だけを更新し、再ビルド・配布・写真選択の変更はしない。以下の旧版説明は当時の記録。

2026-09-08 / Build 13内部配布完了: [保留の距離差診断](WITHHELD-SEPARATION.md)を追加。保存済みの同じ12枚で、保留の「登録した猫に近い／別の猫に近い／同点／比較不能」と方向別の距離比集計を確認できる。判定・モデル・閾値・選択保存・本体は変更しない。集計だけでも各猫1枚ならその1枚の距離比が分かる点をUI/docsに明示。写真/ID/特徴量/個別行は共有しない。

SHA `987896e58913c0b10ce087c193b7e5419508a07f` の専用CI `34197918921` は79テスト成功、合成画面の目視と独立レビュー完了。同一SHAの専用配布 `34198506300` も成功（3分24秒）し、独立app record・署名・archive/IPA検査・Apple検証/アップロード成功を確認。本人の再ログイン後、専用アプリ`6808865207`の0.1 (13)、UUID `ec0d6897-43ca-4fab-941f-f69044988566`の処理「終了」、内部Build 13「テスト中」、既存グループ「自分用（猫識別検証）」、作成16:20を確認。再ビルド・再アップロード・テスター/グループ変更なし。

Build 13での本人操作は完了し、結果を受領済み。新たな選択・交換・判定写真追加は依頼していない。入力は12/12で有効、判定2枚とも期待猫側が近いが距離比不足で保留。精度改善完了ではない。次の比較と判断は冒頭のBuild 14候補を参照。
