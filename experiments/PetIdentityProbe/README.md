...
2026-09-08 / Build 14候補：[候補検索方式の比較と本体採用の判断境界](REFERENCE-RANKING.md)。Build 13本人結果は入力12/12、判定2枚とも期待猫のクラス距離が近いが差不足で保留。モデル作者の公表精度は別の検索方式/評価条件であり、今の5見本・保留付き分類を保証しない。保存済み写真の同じ特徴量で、従来の集約距離と最近傍1枚の順位を一度に比較する。写真の選び直し、追加推論、閾値緩和、本体への自動登録はしない。候補実装中で、配布・本人結果は未確認。以下の旧版説明は当時の記録。
