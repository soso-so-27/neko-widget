...
2026-09-08 / Build 14内部配布完了：[候補検索方式の比較と本体採用の判断境界](REFERENCE-RANKING.md)。Build 13本人結果は入力12/12、判定2枚とも期待猫のクラス距離が近いが差不足で保留。モデル作者の公表精度は別の検索方式/評価条件であり、今の5見本・保留付き分類を保証しない。保存済み写真の同じ特徴量で、従来の集約距離と最近傍1枚の順位を一度に比較する。写真の選び直し、追加推論、閾値緩和、本体への自動登録はしない。

コード `27250d5` の専用CI `34200868316` は85テスト成功・独立レビュー/画面確認完了。同じSHAの配布 `34201596512` が成功。ASCで0.1 (14)、UUID `4c8d6a5a-5302-4be4-90c4-4b7d7ed9c8f7`、作成16:57、処理「終了」、内部「テスト中」、既存グループ「自分用（猫識別検証）」への反映を確認。本人は更新後「保存した12枚で比較する」→「比較結果を共有」だけ。実写の順位比較結果は未確認。以下の旧版説明は当時の記録。
