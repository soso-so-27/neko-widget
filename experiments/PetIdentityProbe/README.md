...
2026-09-08 / Build 8: [同じ実機での基準画像比較](DETECTOR-CONTROLS.md)を追加。生成済み猫画像3枚と保存した猫Aの先頭1枚をボタン1回で比較し、新たな写真選択やモデル実行は不要。元の検出条件は変更しない。実写真の候補0件はBuild 7の形式変換では改善せず、別途CIでは1匹の生成猫に大小2つの候補が出る現象を確認した。自動個体振り分けは引き続き保留。Build 8はApple処理完了・本人用グループで内部テスト中を確認済み。本人の実機比較結果は未確認。以下の旧版の配布説明には当時の状態が含まれるため、最新状態はこの節とBuild 8文書を優先する。
