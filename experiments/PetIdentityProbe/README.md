...
2026-09-08 / Build 9本人結果受領: 同じ保存写真で100%と75%は0候補、50%＋余白では1候補（confidence 0.6135673）。形式統一だけでは検出せず、縮小・余白・補間の組み合わせで結果が変わった。大きさ単独の原因証明や個体識別成功ではない。次の変更は、その50%検出を再利用して元の取得画像へ枠を戻す端末内の切り抜き確認のみ。写真の再選択、追加の検出、保存、個体識別fallbackは行わない。候補`c0deff4`の専用CI `34182202945`で無署名device build・61テストが成功し、生成画像による表示も確認済み。研究ブランチまで反映。TestFlight配布の承認は未受領で、次ビルドは未配布。本人写真での枠の正しさはまだ未確認。詳細は[比較文書の最新追記](DETECTOR-SCALE-COMPARISON.md#build-9本人結果と切り抜き確認候補--2026-09-08)を参照。
