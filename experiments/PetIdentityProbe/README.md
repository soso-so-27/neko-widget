...
2026-09-08 / 次候補: 本人よりBuild 10の切り抜きに猫が写っていることを確認。次は[同じ保存写真による個体判定の比較](IDENTITY-RECOVERY-COMPARISON.md)。元の方法とraw0だけ50%再検出する方法を並行集計し、誤判定増加も見えるようにする。個体判定は新しい研究用ボタンからだけ実行し、本体・旧診断/heldoutの条件・保存済み選択は変更しない。候補実装中、次ビルド未配布。
