...
- Core MLの失敗は`PROBE_COREML_UNVERIFIED`として残す。CPU smokeの成功と、Core ML対応の確認を分ける。CIがgreenでも`summary.json`の`coreml_verified`を確認する。

## 次の実機判断

同じiPhone上でCPU→Core MLを実行し、モデル形式、時間、メモリ、温度状態を確認する。開発試作の段階では一般的な端末の性能を一台から推定しない。最低対応機クラスの実用性が低ければ本体へ追加しない。
