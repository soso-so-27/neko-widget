...
2026-09-08 / Build 11配布済み: 本人よりBuild 10の切り抜きに猫が写っていることを確認し、[同じ保存写真による個体判定の比較](IDENTITY-RECOVERY-COMPARISON.md)を追加。元の方法とraw0だけ50%再検出する方法を集計し、正解・誤判定・保留と見本不足を区別する。「保存したN枚で比較する」→「比較結果を共有」。保存済み選択を再利用し、足りない場合も消さない。本体・旧診断/heldoutの条件は変更しない。SHA `8255d8e`の専用CI `34190017729`は69テスト成功、表示確認・独立レビュー完了。同一SHAの配布 `34190503152`も成功。Apple処理「終了」、内部Build 11「テスト中」、既存本人用グループ・招待1人・アップロード14:28を確認。本人の比較結果は未取得で、識別精度の改善や本体採用は未確定。
